...
2. `mcp__memory__read_graph` — restore rules, preferences, lessons **(daishogun/shogun/karo/gunshi only. ashigaru skip this step — task YAML is sufficient)**
...
4. **Read your instructions file**: daishogun→`instructions/daishogun.md`, shogun→`instructions/shogun.md`, karo→`instructions/karo.md`, ashigaru→`instructions/ashigaru.md`, gunshi→`instructions/gunshi.md`. **NEVER SKIP** — even if a conversation summary exists. Summaries do NOT preserve persona, speech style, or forbidden actions.
...
**daishogunは /clear Recovery ではなく Session Start 手順に従うこと（クロスマシン構成のため）。**

```
Step 1: tmux display-message -t "$TMUX_PANE" -p '#{@agent_id}' → ashigaru{N} or gunshi or daishogun
Step 1.5: 役割確認
  - daishogun → Session Start手順へ（instructions/daishogun.md必須）
...
**クロスマシン配送**: `inbox_write.sh` は `config/topology.yaml` を参照し、ターゲットが別マシンの場合SSH経由で配送する。環境変数 `INBOX_LOCAL_ONLY=1` でフォールバック時のローカルモード強制が可能。
...
|-----------|--------|--------|
| 大将軍 → 将軍 | daishogun_to_shogun.yaml + inbox_write (SSH) | 命令委任 |
| 将軍 → 大将軍 | dashboard.md (git sync) | 結果報告 |
...
| Karo → Shogun | dashboard.md update + inbox_write（cmd完了時） | cmd全subtask完了時に将軍へ報告 |
