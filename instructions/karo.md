...
### Cmd Completion Report to Shogun

cmdの全subtaskが完了したら、dashboard.md更新に加えて将軍にinbox_writeで報告する。
大将軍体制により将軍はLinux独立稼働のため、inbox割り込みの問題は解消された。

**手順（Step 11.7の後に実行）:**
1. dashboard.md更新（従来通り）
2. `bash scripts/inbox_write.sh shogun "cmd_XXX 全subtask完了。dashboard確認されたし。" report_received karo`

| Direction | Method | Reason |
|-----------|--------|--------|
| 家老 → 将軍 | dashboard.md更新 + inbox_write（cmd完了時） | 将軍→大将軍への報告連鎖のため |
